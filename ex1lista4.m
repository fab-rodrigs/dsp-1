% Este script:
%
% Usa a DTFT (função feita a parte, deve estar no mesmo arquivo)
% para obter o espectro contínuo do sinal amostrado.
% Apresenta os gráficos da magnitude e da fase (em graus) em função
% da frequência normalizada.
...
%Caso 1
title('Resposta em magnitude x_1[n] = sin(1000\pi n T_s)', 'T_s = 0.1 ms', 'FontSize', TFS);
...
axis([-1 1 -180 180]);
xlabel('\omega / \pi','FontSize',LFS);
ylabel('Fase (graus)','FontSize',LFS);
% caso 1, mudar o valor de Ts para os demais gráficos
title('Fase x_1[n] = sin(1000\pi n T_s), T_s = 0.1 ms', 'FontSize', TFS);
grid on;
set(gca, 'XTick', -1:0.2:1);
